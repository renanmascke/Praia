...
export async function generateMultiDayRanking(cityName: string, dailyDataBatch: any[]): Promise<AiRankingResult[]> {
    ...
    const buildPrompt = () => `
        ...
        Você receberá, quando disponível, RESUMOS POR PERÍODO (manhã/tarde) dentro de forecast.periods para cada praia.
        ...
        2. Vento (MANHÃ vs TARDE): Use os valores de forecast.periods.morning/afternoon (windSpeedAvg/windDir) quando existirem. Se vento > 8km/h, priorize praias com "Vento Ideal" compatível. Se vento <= 8km/h, considere um dia premium ("mar de espelho").
        3. Mar (MANHÃ vs TARDE): Use waveHeightAvg/waveHeightMax e wavePeriodAvg. Ondas < 0.6m são ótimas para banho. Ondas > 1.2m são perigosas.
        4. Maré (MANHÃ vs TARDE, quando existir): Use tideAvg/tideMin/tideMax para mencionar maré enchendo/secando se fizer sentido, mas não invente se estiver null.
        5. Chuva (CRÍTICO): 
        ...
        - 0 a 8: Vento fraco / Ausente (Dia Premium, mar espelhado).
        - 9 a 18: Vento moderado (Pode incomodar se for de frente/on-shore).
        - 19 a 28: Vento forte (Desconfortável para guarda-sol, especialmente se on-shore).
        - Acima de 28: Vento muito forte (Inviável para banho).

        REGRAS DE VENTO ON-SHORE (CRÍTICO):
        - Ventos soprando DE FRENTE para a praia (Ex: Norte no Norte, Sul no Sul, Leste no Leste) são ruins para o banhista pois trazem marola, vento no rosto e desconforto.
        - Se windSpeedAvg >= 10km/h e a praia não tiver "Vento Ideal" compatível, o score MÁXIMO deve ser 60 (mesmo que a balneabilidade seja própria), e o comentário NÃO deve recomendar essa praia.
        - Priorize SEMPRE praias que estejam abrigadas (Off-shore), onde o vento sopra da areia para o mar. essas devem ter score 85+.

        CONSISTÊNCIA (OBRIGATÓRIO):
        - Você DEVE respeitar os valores numéricos fornecidos (windSpeedAvg/windSpeedMax, waveHeightAvg/waveHeightMax, rainChanceMax, tideAvg/tideMin/tideMax).
        - Se em QUALQUER período (manhã ou tarde) windSpeedAvg >= 20km/h, o comentário deve mencionar vento forte nesse período e NÃO pode sugerir vento fraco/ausente nesse período.
        - Se manhã e tarde forem diferentes, o comentário deve indicar explicitamente qual período é melhor (ex: "de manhã fica redondo..., à tarde o vento aperta").
        - Não invente dados: se um campo vier null, não mencione como se fosse conhecido.
        ...
        [
            {
                ...
                "beachId": "id-da-praia",
                "score": número,
                "commentary": "Uma frase curta e vibrante, consistente com os dados (inclua manhã/tarde quando relevante)."
            },
...
export async function generateCityDailySummary(
    ...
    const buildPrompt = () => `
        ...
        - **DIFERENCIAÇÃO DE PERÍODOS E FOCO POSITIVO**: Diga ao usuário onde está bom HOJE. Se uma região ou período estiver sob vento ou chuva inadequada, NÃO a cite como "bom passeio", apenas sugira a melhor alternativa (ex: "O dia está melhor nas praias do Sul, como Pântano do Sul, fugindo do vento forte do Norte").
        ...
        - **PROIBIDO ALUCINAR**: Use APENAS os nomes de praias fornecidos nos Rankings ou Sugestões.
        - **CONCISÃO**: Máximo 2 parágrafos curtos.
        - **PROIBIDO**: Não se apresente ou use saudações.
        - **CONSISTÊNCIA DE VENTO (OBRIGATÓRIO)**:
          - Se o vento estiver forte/moderado e "on-shore" (de frente) para uma região, você NÃO deve recomendá-la. Ignore-a e se concentre nas outras.
        - **ESCOLHA DE PRAIAS (OBRIGATÓRIO)**: Para manhã/tarde e para o Top 3 final, você DEVE priorizar as praias da lista "Sugestões por Período" que tenham fit "good". 

        ESTRUTURA SUGERIDA:
        1. Resumo do Tempo: Céu, temperatura e risco de chuva.
        2. Dica do Especialista: Recomende as melhores opções para **Manhã** e **Tarde**. Se for o mesmo lugar o dia todo, explique o porquê (ex: "está abrigado"). Comece pelas boas notícias.
           **REGRAS DE VENTO**:
            - Se vento > 20km/h: Chame de "Vento Forte". 
            - Se vento >= 10km/h: NÃO descreva como "vento fraco".
            - **FOCO NO ABRIGO**: Se houver vento frontal no dia, o texto deve começar sugerindo as praias que estão abrigadas.
        3. Top Praias: Destaque as Top 3 do dia utilizando as melhores praias das "Sugestões por Período" (priorizando fit: good). NÃO mencione no ranking praias que você acabou de dizer que estão ruins.

        DADOS:
        - Cidade: ${cityName}
        - Clima Detalhado (Manhã/Tarde/Noite): ${JSON.stringify(weatherData)}
        - Rankings Sugeridos (Use para ver os nomes disponíveis): ${JSON.stringify(rankingData.slice(0, 5))}
        - Referência de Vento do Dia (Manhã/Tarde): ${JSON.stringify(context?.bestAnchorPeriods ?? null)}
        - Sugestões por Período (OBRIGATÓRIO usar nomes daqui):
          - Manhã: ${JSON.stringify(context?.recommended?.morning ?? [])}
          - Tarde: ${JSON.stringify(context?.recommended?.afternoon ?? [])}
