...
export async function generateCityRanking(cityName: string, beachesData: any[]): Promise<AiRankingResult[]> {
    ...
    for (const modelName of modelNames) {
        try {
            ...
            lastError = error;
            // Se for erro de quota (429), não adianta tentar outros modelos na mesma chave geralmente, mas vamos tentar mesmo assim
            if (error.message.includes("429") || error.message.includes("quota")) {
                console.error(">>> ERRO DE QUOTA DETECTADO (429).");
            }
        }
    }
